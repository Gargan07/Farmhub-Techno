...
const DeliveryItem = ({ order }) => {
  ...
  const checkFlair = (percentage) => {
    if (percentage < 50) return "flair danger-flair";
    if (percentage < 90) return "flair warning-flair";
    return "flair success-flair";
  };

  const checkFlairText = (percentage) => {
    if (order.order_cancelled) return "Order Cancelled";
    if (percentage < 50) return "Verification Pending";
    if (percentage < 90) return "Verified & In Delivery";
    return "Delivered";
  };
    ...
    <div className="sub-container delivery-item-container">
        ...
        <div className="delivery-summary">
          ...
          <div className="delivery-date">
            ...
            {(order.order_processed !== true && order.order_cancelled !== true && (
              <h4>{formattedDate}</h4>
            )) || (order.order_processed === true && <h4 className="is-delivered">Delivered</h4>) || 
              (order.order_cancelled === true && <h4 className="is-cancelled">Cancelled</h4>)}
            ...
            <div className="danger-zone">
              ...
              <div className="danger-zone-buttons">
                <button
                  ...
                  onClick={() => {
                    window.location.href = `mailto:www.minisylar3@gmail.com?subject=Reporting Order #${order._id.slice(0, 6)}`;
                  }}
                >
                  Report Issue
                </button>
      ...
      <div className="expand-collapse-delivery">
        <button onClick={toggleExpanded}>
          ...
          <span>
            <FaCaretUp className={expanded ? "caret-delivery" : "caret-delivery caret-expanded"} />
          </span>
        </button>
      </div>
    </div>
